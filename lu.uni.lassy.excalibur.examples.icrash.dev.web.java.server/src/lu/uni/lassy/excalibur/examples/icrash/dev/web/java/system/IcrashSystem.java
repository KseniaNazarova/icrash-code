...
// it is OK that way for an educative program... but surely not OK for a realworld production one!
	...
	/**  A hashtable of the media in the system, stored by their ID as a key. */
	static Hashtable<String, CtMedia> cmpSystemCtMedia = new Hashtable<String, CtMedia>();
	...
	public PtBoolean oeShareByCrisis(DtCrisisID aDtCrisisID) {
		try{
			// PreP 1
			if (!isSystemStartedCheck()) {
				log.debug("Inside oeShareByCrisis: the system is not started! Aborting...");
				return new PtBoolean(false);
			}
			...
			log.error("Exception in oeShareByCrisis..." + e);
		}
		return new PtBoolean(true);
	}
	...
	public PtBoolean oeRemoveMedia(DtMediaID aDtMediaID){
		try{
			// PreP 1
			if (!isSystemStartedCheck()) {
				log.debug("Inside oeRemoveMedia: the system is not started! Aborting...");
				return new PtBoolean(false);
			}
